...
def on_key_press(event):
    ...
    if not recording:  # Éviter de redémarrer si déjà en cours
        ...
        text_dynamic.set_text("Enregistrement en cours... Maintenez la touche/clic.")


def on_key_release(event):
    """Callback pour arrêter l'enregistrement et sauvegarder les données."""
    global recording, notes_values, counter
    if recording:
        recording = False
...
def update(frame):
    ...
    notes_display.set_text(f"Notes enregistrées:\n{notes_str}")
