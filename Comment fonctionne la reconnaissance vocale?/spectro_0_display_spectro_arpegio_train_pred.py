...
file_names = ["D-7b5_arpegio", "G_arpegio", "C-_arpegio"]
labels = ['Arpege: Ré mineur 7 b5 (D-7b5)', 'Arpege: Sol Majeur (G)', 'Arpege: Do mineur (C-)']
for label_index, file_name in enumerate(file_names):
    print(f"Chargement des données pour {labels[label_index]}...")
    print(label_index)
    for i in range(3):
        notes_values = np.loadtxt(f"./chords/{file_name}_{i}.txt")
        X.append(notes_values / divider)
        y.append(label_index)

mlp = MLPClassifier(hidden_layer_sizes=(256, 128, 128), max_iter=1500, alpha=1e-4, solver='adam', activation='relu')
mlp.fit(X, y)
...
def on_key_release(event):
    ...
    global recording, notes_values, counter, mlp, divider,timer,local_timer
    if recording:
        recording = False
        text_dynamic.set_text("")
        if notes_values:
            ...
            notes_values = np.array(notes_values[:1024])
            ...
            timer = threading.Timer(5, create_timer)
            timer.start()
