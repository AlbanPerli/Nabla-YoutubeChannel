...
file_names = ['Misère', 'Ingratitude', 'Profit', 'Egoisme', "Oportunisme"]
labels = ['Misère', 'Ingratitude', 'Profit', 'Egoisme', "Oportunisme"]
for label_index, file_name in enumerate(file_names):
    print(f"Chargement des données pour {labels[label_index]}...")
    print(label_index)
    for i in range(4):
        notes_values = np.loadtxt(f"./words/{file_name}_{i}.txt")
        X.append(notes_values / divider)
        y.append(label_index)

mlp = MLPClassifier(hidden_layer_sizes=(1024,256,64), max_iter=1000, alpha=1e-4, solver='adam', activation='relu')
mlp.fit(X, y)
...
def on_key_release(event):
    ...
    if recording:
        ...
        if notes_values:
            ...
            notes_values += [0] * (2000 - len(notes_values))
            # retirer les valeurs au delà de 40 éléments
            notes_values = np.array(notes_values[:2000])/divider
            print(notes_values)
