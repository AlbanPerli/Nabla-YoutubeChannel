...
def update(frame):
    ...
    bin_size = 5  
    num_bins = len(fft_data) // bin_size
    fft_binned = np.array([np.mean(fft_data[i*bin_size:(i+1)*bin_size]) for i in range(num_bins)])
    
    frequencies_binned = freqs[:num_bins] * bin_size
    
    line_fft.set_ydata(fft_data)
    
    # 242, 248, 255, 262, 269 do
    # 296, 302, 309, 316, 322 mib
    # 255, 262, 302, 309, 316 interval do mib == addition de do et mib
    
    spectrogram_data = np.roll(spectrogram_data, -1, axis=1)
    spectrogram_data[:, -1] = fft_data
    # extract the top 10 most intense frequencies
    top_10_freqs = np.argsort(fft_binned)[-10:]
    # print(fft_binned[top_10_freqs[0]])
    if fft_binned[top_10_freqs[0]] > 0.5:
        top_10_freqs = [int(f) for f in frequencies_binned[top_10_freqs]]
        # sort the frequencies
        top_10_freqs.sort()
        signal.append(top_10_freqs)
    else:
        signal.append([-1])
